...
def process_cmd(key):
    global crop, black_background, offset_x, offset_y, scale_x, scale_y
    scale_amount = 0.005
    offset_amount = 5

    def print_scale():
        print("ox={} oy={} sx={} sy={}".format(
...
def main():
    # imagePath="1913.jpg"
    thresh = 0.1
    configPath = "./config/puck.cfg"
    weightPath = "./config/puck.weights"
    metaPath = ".config//puck.data"

    # Import the global variables. This lets us instance Darknet once, then just call performDetect() again without instancing again
    global metaMain, netMain, altNames, crop, crop_baseline, rotate, offset_x, offset_y, scale_x, scale_y  # pylint: disable=W0603
    if netMain is None:
        ...
        if frame is not None:
            ...
            if engaged:
                ...
                for detection in detections:
                    label, confidence, boundingBox = detection_to_puck(
                        detection)
...
#            image = image[crop[0]:image.shape[0]-crop[1],crop[2]:image.shape[1] - crop[3]]
            ...
            cv2.putText(image, str(crop), (10, 30),
                        cv2.FONT_HERSHEY_PLAIN, 1, (255, 0, 0), 2)
            image = cv2.resize(image, (1280, 800),
                               interpolation=cv2.INTER_CUBIC)
            resized = cv2.resize(
                image, (int(1280*scale_x), int(800*scale_y)), interpolation=cv2.INTER_CUBIC)
            cropped = resized[offset_y:offset_y+800, offset_x:offset_x+1280]
