...
class Robot:
    ...
    def goto(self, x, y, wait=False):
        ...
        payload = self.mode + self.target_x_high + self.target_x_low + self.target_y_high + self.target_y_low + self.speed_high + self.speed_low + \
            self.accel_high + self.accel_low + self.current_pos_x_high + \
            self.current_pos_x_low + self.current_pos_y_high + self.current_pos_y_low
        self.sock.sendto(payload, (self.ipaddress, self.port))
        if wait:
            old_x = ord(self.current_pos_x_high) * \
                255 + ord(self.current_pos_x_low)
            old_y = ord(self.current_pos_y_high) * \
                255 + ord(self.current_pos_y_low)
    ...
    def setup_angled_shot(self, puck):
        ...
        starting_point = (
            int(unit_vector[0] * -50) + puck[0], int(unit_vector[1] * -50) + puck[1])
        print(starting_point)
        self.goto(starting_point[0], starting_point[1], True)
        end_point = (int(unit_vector[0] * 150) + puck[0],
                     int(unit_vector[1] * 150) + puck[1])
        end_point = (
            int((end_point[1] - 350) * unit_vector[1] / unit_vector[0] + end_point[0]), 350)
        print(end_point)
        self.goto(end_point[0], end_point[1], True)
        self.goto(265, 0, True)

    def __str__(self):
        return "Positronic's Air Hockey Table Robot"
