...
void packetRead()
{
  ...
  if (Serial1.available() > 0) {
    ...
    if (readStatus == 2)
    {
      ...
      {
        // CHECK MAXIMUNS
        user_target_x = extractParamInt(10);
        user_target_y = extractParamInt(8);
...
// We configure the ESP8266 to generate a POSITRONIC_xx Wifi and Listen UDP messages on port 2222 (_xx depends on your robot)
void ESPInit()
{
  ...
  // Generate Soft AP. SSID=POSITRONIC, PASS=87654321
  const char *cmd =  "AT+CWSAP=\"POSITRONIC_ROBOT\",\"87654321\",5,3";
